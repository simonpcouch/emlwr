...
I'll first run this experiment for numbers of rows $100, 1000, ..., 1,000,000$ both sequentially and via socket clusters on my laptop, recording the timings as I do so. Then, I'll do the same thing on a hosted service, and we'll compare results.
...
There's one timing per unique combination of number of rows, parallelism plan, and system. Based on these timings, we can calculate the factor of speedup for sequential evaluation versus its parallel analogue.
