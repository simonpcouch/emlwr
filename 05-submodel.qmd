...
First, loading in data and setting up the model specification as before:
...
This grid is "regular" in the sense that `levels = 4` values are generated individually for each parameter, and the resulting grid is created by pairing up each unique combination of those values.
...
if (identical(eval_fits, "true")) {
  save(bm_grid_regular, file = "data/05-submodel/bm_grid_regular.Rda")
} else {
  load("data/05-submodel/bm_grid_regular.Rda")
}
```
...
Changing only the `grid` argument (and even increasing the number of proposed `grid` points), we've decreased the time to evaluate against resamples from `r round(as.numeric(bm_basic$median[[1]]) / 60 / 60, 2)` to `r round(as.numeric(bm_grid_regular$median[[1]]) / 60 / 60, 2)`.
...
bm_grid_regular_go_brrr <-
  ...
  map(1:max(.$trees), ~mutate(., trees = .x))

bm_grid_regular_go_brrr
```

In this grid, we have the same number of unique values of `learn_rate`, $4$, resulting in the same $4$ model fits. Except that, in this case, we're evaluating every model with number of trees $1, 2, 3, 4, ..., 2000$. If our hypothesis that predicting on the assessment set and generating performance metrics is comparatively fast is true, then we'll see that elapsed time *per grid point* is way lower:
...
#| include: false
if (identical(eval_fits, "true")) {
  save(bm_grid_regular_go_brrr, file = "data/05-submodel/bm_grid_regular_go_brrr.Rda")
} else {
  load("data/05-submodel/bm_grid_regular_go_brrr.Rda")
}
```

The above code took `r round(as.numeric(bm_grid_regular_go_brrr$median[[1]]) / 60 / 60, 2)` hours to run, definitely longer than the `r round(as.numeric(bm_grid_regular$median[[1]]) / 60 / 60, 2)` hours from `bm_grid_regular` (that fitted the same number of models), but comparable. But, *per grid point,* that difference is huge:
